...
public class Genetic {
    ...
    public static final double PERCENT_SELECTION = .7; // this portion of the children will be selected using tournament
                                                       // selection and the rest are randomly generated to increase
                                                       // diversity
    public static final double ERROR_PORTION = .25; // portion of dataset that's being checked to avoid overfitting
                                                    // hasn't been implemented yet, overfitting hasn't yet been an issue
    public static final double MUTATION_RATE = .95; // what Math.Random has to beat for a mutation
    public static final double UPSET_RATE = .95; // what Math.Random has to beat to allow an upset in tournament
                                                 // selection
    public static final double TOURNAMENT_PERCENT = .2; // percent of population which participates in tournament
                                                        // selection
    ...
    public static Tree geneticAlgorithm(ArrayList<ArrayList<String>> data, boolean bonusFields, boolean bonusOperators) {
        ...
        for (int i = 0; i < GENERATIONS; i++) {
            ...
            System.out.println("Current Generation: " + i + " with fitness " + best.fitness + " from function "
                    + best.inOrderTraverse() + " and size " + children.size());
            parents = children;
            if (best.fitness == 0)
                break;
        }
    ...
    public static Tree[] tournamentSelect(ArrayList<Tree> trees, int tournamentSize,
    ...
    public static double evaluate(ArrayList<ArrayList<String>> data, Tree tree, int cap) {
        ...
        for (int i = 1; i < cap; i++) {
            if (!tree.bonusFields) {
                ...
                double MSE = ((actual - guess) * (actual - guess));
        ...
        return computationErrors < cap / 2 ? error / (cap - computationErrors) : error / 0;
